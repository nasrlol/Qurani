...
import {changeLanguage, filterSearch, getSurahListOption, getSurahTitle, getSurahTranslated} from "../api/surah.mjs";
...
export async function render(container) {
    ...
    const main = `
         <main>
            <div class="surahFields">
                <input id="searchInput" class="searchInput" placeholder="Search for a translation...">
                <div>
    ...
    for (let i = 0; i < options.length; i++) {
        ...
        const title = await getSurahTitle(id);
        const surahTranslated = await getSurahTranslated(id);
